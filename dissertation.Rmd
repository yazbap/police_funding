...
countes_pol_ideo <- read_dta("data/aip_counties_ideology_v2022a.dta")
...
counities_pol_ideo <- countes_pol_ideo %>%
  filter(survey_period == "2017-2021") %>% #Only use data from most recent surveys
  mutate(state_abbr = state.abb[match(state, state.name)], #get abbreviated states
...
cities_demo <- cities_demo %>%
  ...
  mutate(percent_female = num_female / total_pop,
         ...
         PLACE = tolower(gsub(" County| Borough| Census Area| Municipality| Parish| city", "", PLACE)), #remove suffixes
...
da38651.0001_filtered <- da38651.0001 %>%
  ...
  dplyr::select(AGENCYNAME, AGENCYSAMPTYPE, CITY, STATE, OPBUDGET_2019,
                OPBUDGET) %>%
