...
```{r create_cache_database}
# Assign the directory and database name
cache_database <- 'cache_database/policing_data.db' 

# Ensure the directory exists (create it if it doesn't)
if (!dir.exists("cache_database")) {
  dir.create("cache_database")
}

# Connect to the SQLite database
db <- dbConnect(RSQLite::SQLite(), cache_database)

# Check if the database file exists
file.exists(cache_database) # This should return TRUE if the database was created
...
process_and_plot <- function(df, removed_rows, remove_col) {
  ...
  # Create a named vector for mapping variables to their labels
  var_labels <- c(
  ...
  binary_means <- df %>%
    ...
    pivot_longer(cols = -removed_status, names_to = "variable", values_to = "mean") %>%
    mutate(variable = paste(var_labels[variable], "(binary)"))
  ...
  plot <- ggplot(combined_means, aes(x = mean, y = variable, fill = removed_status)) +
    ...
    labs(x = "Mean", y = "Variable", fill = "Removed Status") +
...
```{r get_protest_data}
# Load protest data
raw <- read.csv("data/protests/2020-01-01-2024-04-19-United_States.csv")
...
cities_pol_ideo <- read_dta("data/ideology/aip_cities_ideology_v2022a.dta")
counties_pol_ideo <- read_dta("data/ideology/aip_counties_ideology_v2022a.dta")
zips_pol_ideo <- read_dta("data/ideology/aip_zips_ideology_v2022a.dta")
...
cities_demo <- read.csv("data/demographics/nhgis0006_ts_nominal_place.csv")
counties_demo <- read.csv("data/demographics/nhgis0004_ts_nominal_county.csv")
...
police_killings_df <- suppressWarnings(
  suppressMessages(
    read_excel("data/police_killings/police_killings.xlsx", sheet = "2013-2024 Police Killings")
  )
)
...
location_df <- read_csv("data/funding/location_corrections.csv")
budgets <- read_csv("data/funding/budget_corrections.csv")
...
police_df <- full_police_df %>%
  ...
  mutate(zip = as.character(zip)) %>%
  select(
    AGENCYNAME, location, is_city, STATE, zip, where(~ all(!is.na(.)))
  ) %>%
  left_join(budgets, by = c("AGENCYNAME", "STATE")) %>%
  select(-source_budget_2021)
...
```{r save_pre_imputation_df}
db <- dbConnect(RSQLite::SQLite(), cache_database) # Connect
#dbWriteTable(db, "df_pre_imputation", df_pre_imputation) # Write
dbDisconnect(db) # Disconnect

db <- dbConnect(RSQLite::SQLite(), cache_database) # Connect
df_pre_imputation <- dbGetQuery(db,
                           "SELECT *
                           FROM df_pre_imputation") # Conduct query
dbDisconnect(db) # Disconnect
```


```{r impute, eval = FALSE}
full_df_imputed <- missForest(df_pre_imputation, replace = TRUE)

OOB_error <- full_df_imputed$OOBerror

final_df <- full_df_imputed$ximp

df_pre_imputation <- as.data.frame(df_pre_imputation)

df_post_imputation <- cbind(df_cleaned$AGENCYNAME, df_cleaned$City, df_cleaned$STATE,
                      final_df)

```

```{r save_post_imputation_df}
db <- dbConnect(RSQLite::SQLite(), cache_database) # Connect
#dbWriteTable(db, "df_post_imputation", df_post_imputation) # Write
dbDisconnect(db) # Disconnect

db <- dbConnect(RSQLite::SQLite(), cache_database) # Connect
df_post_imputation <- dbGetQuery(db,
                           "SELECT *
                           FROM df_post_imputation") # Conduct query
dbDisconnect(db) # Disconnect
```
...
set.seed(123)
mice_imp <- mice(df_pre_imputation, m = num_iterations, method = 'rf', maxit = 50, seed = 123)
...
for (i in 1:num_iterations) {
  imputed_missForest_list[[i]] <- missForest(df_pre_imputation)$ximp
  if (i > 1) {
...
mean_range <- calculate_range(df_pre_imputation)
...
save(figure1, file = "tables_figures/figure1.rda")

```


```{r imputation_summary_stats, fig.height = 10, fig.width = 10}
columns_to_show <- c("budget_2019", "budget_2020", "budget_2021")
custom_labels <- c("budget_2019" = "2019 Budget", "budget_2020" = "2020 Budget",
                   "budget_2021" = "2021 Budget"
                   )
...
for (col_name in columns_to_show) {
  ...
  if (col_name %in% names(df_post_imputation) && col_name %in% names(df_pre_imputation) && is.numeric(df_post_imputation[[col_name]]) && is.numeric(df_pre_imputation[[col_name]])) {
    # Calculate and format summary statistics for the imputed and original data
    imputed_stats <- compute_summary_stats(df_post_imputation[[col_name]])
    original_stats <- compute_summary_stats(df_pre_imputation[[col_name]])
...
save(summary_stats, file = "tables_figures/summary_stats.rda")
...
# Generate tables
balance_pre_match_peaceful <- create_result_table("peaceful_protest", df_post_imputation)
save(balance_pre_match_peaceful, file = "tables_figures/balance_pre_match_peaceful.rda")

balance_pre_match_violent <- create_result_table("violent_protest", df_post_imputation)
save(balance_pre_match_violent, file = "tables_figures/balance_pre_match_violent.rda")

```

```{r normalize_data}
skewed_data <- df_post_imputation %>%
...
df_post_imputation <- as.data.frame(df_post_imputation)
normalized_data <- as.data.frame(normalized_data)

# Merge county_state column from full_protest_data with normalized_test
normalized_df <- cbind(agency_name = df_post_imputation$`df_cleaned$AGENCYNAME`, 
                       city = df_post_imputation$`df_cleaned$City`,
                       state = df_post_imputation$`df_cleaned$STATE`,
                       opbudget2013 = df_post_imputation$budget_2013,
                       opbudget2016 = df_post_imputation$budget_2016,
                       opbudget2019 = df_post_imputation$budget_2019,
                       opbudget2020 = df_post_imputation$budget_2020,
                       opbudget2021 = df_post_imputation$budget_2021,
                       is_city = df_post_imputation$is_city,
                       has_police_killing = df_post_imputation$has_police_killing,
                       violent_protest = df_post_imputation$violent_protest,
                       peaceful_protest = df_post_imputation$peaceful_protest,
                       normalized_data)

normalized_df$opbudget2013 = log(normalized_df$opbudget2013)
normalized_df$opbudget2016 = log(normalized_df$opbudget2016)
normalized_df$opbudget2019 = log(normalized_df$opbudget2019)
normalized_df$opbudget2020 = log(normalized_df$opbudget2020)
normalized_df$opbudget2021 = log(normalized_df$opbudget2021)

```

```{r save_normalized_data}
db <- dbConnect(RSQLite::SQLite(), cache_database) # Connect
#dbWriteTable(db, "normalized_df", normalized_df) # Write
dbDisconnect(db) # Disconnect

db <- dbConnect(RSQLite::SQLite(), cache_database) # Connect
normalized_df <- dbGetQuery(db,
                           "SELECT *
                           FROM normalized_df") # Conduct query
dbDisconnect(db) # Disconnect
...
max_opbudget2019 <- max(normalized_df$opbudget2019[!is.infinite(normalized_df$opbudget2019)], na.rm = TRUE)
max_opbudget2020 <- max(normalized_df$opbudget2020[!is.infinite(normalized_df$opbudget2020)], na.rm = TRUE)
max_opbudget2021 <- max(normalized_df$opbudget2021[!is.infinite(normalized_df$opbudget2021)], na.rm = TRUE)

normalized_df$opbudget2019 <- ifelse(is.infinite(normalized_df$opbudget2019), 
                                     max_opbudget2019, normalized_df$opbudget2019)
normalized_df$opbudget2020 <- ifelse(is.infinite(normalized_df$opbudget2020), 
                                     max_opbudget2020, normalized_df$opbudget2020)
normalized_df$opbudget2021 <- ifelse(is.infinite(normalized_df$opbudget2021), 
                                     max_opbudget2021, normalized_df$opbudget2021)
```

```{r conduct_DID_pre_match}
did_pre_matching_peaceful <- prepare_and_save_diff_in_diff(normalized_df, 
                                                           "peaceful_protest", 
                                                           "opbudget2019", 
                                                           "opbudget2021")
save(did_pre_matching_peaceful, file = "tables_figures/did_pre_matching_peaceful.rda")

did_pre_matching_violent <- prepare_and_save_diff_in_diff(normalized_df, "violent_protest", 
                                        "opbudget2019", "opbudget2021")
save(did_pre_matching_violent, file = "tables_figures/did_pre_matching_violent.rda")
...
removed_peaceful <- process_and_plot(
  df = normalized_df,
...
removed_violent <- process_and_plot(
  df = normalized_df,
  removed_rows = removed_rows_violent,
  remove_col = "violent_protest"
)

save(removed_peaceful, file = "tables_figures/removed_peaceful.rda")
save(removed_violent, file = "tables_figures/removed_violent.rda")
```

```{r generate_post_matched_tables}
balance_post_match_peaceful <- create_result_table("peaceful_protest", m_data_peaceful)
save(balance_post_match_peaceful, file = "tables_figures/balance_post_match_peaceful.rda")

balance_post_match_violent <- create_result_table("violent_protest", m_data_violent)
save(balance_post_match_violent, file = "tables_figures/balance_post_match_violent.rda")
```

```{r conduct_DID_post_match}
did_post_matching_peaceful <- prepare_and_save_diff_in_diff(m_data_peaceful, "peaceful_protest", 
                              "opbudget2019", "opbudget2021")
save(did_post_matching_peaceful, file = "tables_figures/did_post_matching_peaceful.rda")

did_post_matching_violent <- prepare_and_save_diff_in_diff(m_data_violent, "violent_protest", 
                              "opbudget2019", "opbudget2021")
save(did_post_matching_violent, file = "tables_figures/did_post_matching_violent.rda")
...
save(parallel_peaceful, file = "tables_figures/parallel_peaceful.rda")

# For violent protests
parallel_violent <- run_parallel_trends(data = m_data_violent, protest_var = "violent_protest")
save(parallel_violent, file = "tables_figures/parallel_violent.rda")
```
